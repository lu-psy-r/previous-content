...
1. Whether an effect is or is not significant in the data-sets we are comparing;
2. Whether the estimate of the coefficient for the slope of an effect of interest has similar sign (positive or negative) or size (the value of the coefficient) in the data-sets we are comparing.
:::
...
3. Get the 2024-25 PSYC122 `2024-25_PSYC122-participants.csv` data

```{r eval=FALSE}
download.file("https://github.com/lu-psy-r/statistics_for_psychologists/blob/main/PSYC122/data/week19/2024-25_PSYC122-participants.csv?raw=true", destfile = "2024-25_PSYC122-participants.csv")
```
::: 
...
::: callout-tip
Note that the data we collected comprise responses from 2024-25 PSYC122 students --- **you** --- as well as from samples of participants recruited by Psychology third year undergraduate researchers.
:::
...
4. Inspect both data-sets -- using `head()` and `summary()`
...
9. Create grids of plots to make side-by-side comparisons of associations in different data-sets
...
12. Do this in different -- but similar -- study data-sets, and compare summaries or plots
...
::: callout-warning
Don't forget: You will need to change the names of the data-set or the variables to complete the tasks in @sec-wk19-lab-activities-2.
:::
...
Use the `summary()` or `head()` functions to take a look at both data-sets.
...
> Hint: Task 6 -- What we are going to do is to create two histograms of `mean.acc` (one plot for each study data-set) and then present the plots side by side to allow easy comparison of variable distributions

We need to make two changes to the coding approach you have been using until now.

- Remember, you can find what you need to do in `PSYC122-w19-how-to.Rmd`.
...
Do this process twice, once for each data-set, creating two plots so that you can compare the distribution of `mean.acc` scores between the studies.
...
##### Task 8 -- Draw scatterplots to compare the potential association between `mean.acc` and `mean.self` in both `study.two.gen` and `study.122` data-sets
...
##### Task 10 --  Can you estimate the association between `mean.acc` and `mean.self` in both data-sets?

> Hint: Task 10 -- We use `cor.test()` as you have been shown how to do e.g. in `PSYC122-w16-how-to.Rmd`

Do the correlation for both data-sets.
...
>**Q.13.**  Can you compare the estimates, given the two data-sets, to evaluate if the result in `study.two.gen` is *replicated* in `study.122`?
...
> Hint: Task 12 -- We use `lm()`, as we have been doing before, see e.g. `PSYC122-w18-how-to.R`
...
```{r}
dat.two <- ggpredict(model.two, "HLVA")
plot.two <- plot(dat.two, colors = "bw") + labs(title = "Study Two")
dat.122 <- ggpredict(model.122, "HLVA")
plot.122 <- plot(dat.122, colors = "bw") + labs(title = "Study PSYC122")
```
:::
...
<!-- 1. There are far fewer observations in the `study.122` data-set than in the `study.two.gen` data: this means that our estimate of the effect will be *more uncertain* because we have *less information* when we look at the `study.122` data. -->
...
Draw scatterplots to compare the potential association between `mean.acc` and `mean.self` in both the `study.two.gen` and the `study.122` data-sets.
...
Can you estimate the association between `mean.acc` and `mean.self` in both data-sets?

- We use `cor.test()` as you have been shown how to do e.g. in `PSYC122-w16-how-to.Rmd`.
- Do the correlation for both data-sets.
...
We use `lm()`, as we have been doing before, see e.g. `PSYC122-w18-how-to.R`
