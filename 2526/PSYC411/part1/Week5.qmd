...
```{=html}
<iframe src="https://lancaster.cloud.panopto.eu/Panopto/Pages/Embed.aspx?id=98f7b065-26f1-42d9-9872-ac5900c7cda0&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=true&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay" aria-label="Panopto Embedded Video Player" aria-description="psyc411_2025_w5_p1" ></iframe>
```

Watch Lecture week 5 part 2:

```{=html}
<iframe src="https://lancaster.cloud.panopto.eu/Panopto/Pages/Embed.aspx?id=551cef0e-76e4-43b6-8895-ac5900cdf342&autoplay=false&offerviewer=true&showtitle=true&showbrand=true&captions=true&interactivity=all" height="405" width="720" style="border: 1px solid #464646;" allowfullscreen allow="autoplay" aria-label="Panopto Embedded Video Player" aria-description="psyc411_2025_w5_p2" ></iframe>
```

Take [the quiz](https://modules.lancaster.ac.uk/mod/quiz/view.php?id=2680955){target="_blank"} (not assessed) on the lecture materials.
...
28. The key part of the results to look at is the one that has t = -1.7019, df = 100.37, p-value = 0.09187. This is the result that you report: *t*(100.37) = -1.70, *p* = .092.
...
terms of vocabulary scores. Males (mean = 62.33, SD = 13.74) scored higher than 
females (mean = 58.99, SD = 13.72) on the first time participants attempted the 
Gent vocabulary test, however this difference was not significant,
*t*(100.37) = -1.70, *p* = .092, Cohen's *d* = 0.24. We did not find evidence to support the hypothesis.
...
Native
English speakers (mean = 65.14, SD = 9.14) scored significantly higher
on the Gent vocabulary test than speakers of English as a second
language (mean = 50.05, SD = 15.31), *t*(142.81) = -9.08, *p* < .001,
Cohen's *d* = 1.20, a large effect size. Thus, the hypothesis was supported, with native English speakers scoring significantly higher on the Gent vocabulary test than those with English as an additional language.
...
## Answer
``` {r}
summarydata <- select(all_dat, id, ahiTotal, occasion)
```
:::
...
## Answer
``` {r}
summarydata3 <- group_by(summarydata2, occasion)
summarise(summarydata3, mean(ahiTotal), sd(ahiTotal), n())
# or:
group_by(summarydata2, occasion) %>% summarise(mean(ahiTotal), sd(ahiTotal), n())
```
:::
...
58. In the Gent vocabulary scores data, is there a significant difference between native and ESL speakers for your academic year group? 

::: {.callout-note icon="false" collapse="true"}
## Answer
``` {r}
dat <- read_csv("data/wk4/PSYC411-shipley-scores-anonymous-17_25_year_language.csv")
dat2 <- filter(dat, academic_year == "202526")
ggplot(dat2, aes(x = english_status, y = Gent_1_score)) + geom_boxplot()
dat3 <- group_by(dat2, english_status)
summarise(dat3, mean(Gent_1_score), sd(Gent_1_score), mean(Gent_2_score), sd(Gent_2_score), n())
# or:
dat2 %>% group_by(english_status) %>% summarise(mean(Gent_1_score), sd(Gent_1_score), mean(Gent_2_score), sd(Gent_2_score), n())
...
59. Are there significant differences for the Shipley vocabulary test measures between males and females, or between those with English as first or second language?

::: {.callout-note icon="false" collapse="true"}
## Answer
``` {r}
dat3 <- group_by(dat2, english_status)
summarise(dat3, mean(Shipley_Voc_Score, na.rm=TRUE), sd(Shipley_Voc_Score, na.rm=TRUE), n())
# or:
dat2 %>% group_by(english_status) %>% summarise(mean(Shipley_Voc_Score, na.rm=TRUE), sd(Shipley_Voc_Score, na.rm=TRUE), n())
...
60. The data from this paper are called [ClassDraw.csv available here](data/wk5/ClassDraw.csv). The data are on osf as well, but they're not formatted in a very accessible way, so I adjusted them and put them here.
...
The answers to the workbook appear below each question in the workbook, above, so you can check your work *after you've had a go at the exercises..!*.

## 5.5 Extras

Optionally, if you can give us your (anonymised) [feedback](https://modules.lancaster.ac.uk/mod/feedback/view.php?id=2680956){target="_blank"} on how the course is going from your perspective, that would be very welcome.
