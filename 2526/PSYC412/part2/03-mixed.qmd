...
3. This week, you can practical exercies in the [chapter on developing linear mixed-effects models](../part2/03-mixed-workbook.qmd#sec-dev-mixed-workbook-overview).
...
4.  There is no sign of the presence of missing values in this data-set, no counts of `NAs`.

We do not really want R to treat a coding variable like `item_number` as numeric: it functions as a categorical or nominal variable, a factor. And we want R to treat coding variables like `subjectID` as factors. In [the Introduction to multilevel data chapter section on *coercion*](../part2/01-multilevel.qmd#sec-multi-data-coerce), we saw how we can require R to handle variables exactly as we require it to using *coercion*. In [the Introduction to linear mixed-effects models chapter section on data loading](../part2/02-mixed.qmd#sec-intro-mixed-data-load), we saw how we can determine how R treats variables at the read-in stage, using `col_types()` specification. We are going to do neither here because we do not have to do this work; not doing it will have no impact on our analyses at this point.
...
You can read a brief discussion of the impacts of researcher choices in data-set construction in @steegen2016.
...
1.  `ML.all.correct %>%` takes the selected filtered data-set `ML.all.correct` and pipes it `%>%` to the next step.
...
4.  Using `ungroup() %>%` means that, having grouped the data to calculate the mean log RTs, we `ungroup` the data-set so that R can look at all observations in the next step.
5.  `mutate(subjectID = fct_reorder(subjectID, mean_logrt)) %>%` asks R to look at all log RT observations in the data-set, and change the top-to-bottom order of the rows.
...
1.  `geom_vline(xintercept = 2.778807, colour = "red", linetype = 2)` draws a vertical red dashed line at the location of the mean log RT, the average of all log RTs over all participants in the data-set.
...
As we saw, in a similar analysis in [the Introduction to linear mixed-effects models chapter](../part2/02-mixed.qmd#sec-intro-mixed), we can estimate the relationship between reading reaction times (here, lexical decision RTs) and word frequency using a linear model:
...
As we know, some words elicit slower and some elicit faster responses on average. As we discussed in the last chapter,
in [the Introduction to linear mixed-effects models chapter section on the impact of stimulus variation](../part2/02-mixed.qmd#sec-intro-mixed-fixed-effect-fallacy), if we did not take such variation into account, we might spuriously identify an experimental effect actually due just to unexplained between-items differences in intercepts [@clark1973; @raaijmakers1999] committing an error: *the language as fixed effect fallacy*.
...
What is the impact of the incorporation of random effects -- the variance and covariance terms -- in mixed-effects models? Mixed-effects models can be understood, in general, as a method to compromise between ignoring the differences between groups (here, participants constitute groups of data) as in *complete pooling* or focusing entirely on each group (participant) as in *no pooling* [@Gelman2007ga]. In this discussion, I am going to refer to the differences between participants but you can assume that the lesson applies generally to any situation in which you have different units in a multilevel structured data-set in which the units correspond to groups or clusters of data.
...
# -- update the previous plot by using a data-set consisting of of all three models’ estimates
...
For the @Davies2013b data-set, I include first the set of phonetic coding variables then the set of psycholinguistic variables (see the paper for details).
